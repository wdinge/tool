...
History:         ['#051020','History',         '历史'],
Literature:      ['#051020','Literature',      '文学'],
Major_Prophets:  ['#051020','Major_Prophets',  '大先知'],
Minor_Prophets:  ['#051020','Minor_Prophets',  '小先知'],
NT:              ['#051020','N.T.',            '新约全书'],
Gospels:         ['#051020','Gospels',         '四福音书'],
HisSayings:      ['#051020','HisSayings',      '耶稣话语'],
Epistles:        ['#051020','Epistles',        '书信'],
Pauls:           ['#051020','Pauls',           '保罗书信'],
};
...
BookChapterVerse.prototype.Init=function(){
    ...
    //$("#selBookId optgroup option").each(function(){
    //    var skey=$(this).val();
    //    var sNam=$(this).text();
    //    sNam=$.trim(sNam);
...
function StartToGo( bForceReload){
    ...
    var sBibleName = MyCookies.get("selMainBible","CUVs");//$("#selMainBible").val(); 
...
function body_onload() {
    ...
    var sBKeyId=MyCookies.get("sBKeyId","_Gen1_1");
...
function updateFontSize(){
    var fontsize = MyCookies.get("fontsize","90");
...
function construeItem_OnClick() {//on td 0 clicked, change the translation. 
    ...
    default:
        ...
        var txt=$("#"+txtid).text();    
        var bible=MyCookies.get("selMainBible","CUVs");//$("#selMainBible").val();
    ...
    //
    var csid = BookChapterVerseUti.construedId(sky);
    var fntsize = MyCookies.get("fontsize","90");
    $("#"+csid).html( txt )
...
function GetCurBible() {
    var BibleVersion = MyCookies.get("selMainBible","CUVs");//$("#selMainBible").val(); //GetBibleVersionThruWinLoc();
...
function ajxGetLoginInfo(){ //csid,sky   
    ...
    $.ajax({
      ...
      success: function(ret) {
		  ret=ret.trim();
		  var eml=MyCookies.get("email","--");
		  console.log("email:"+eml);
          console.log("ajxGetLoginInfo:"+ret);
		  gLoginUserEmail="";
		  var idxpos = ret.indexOf("Invalid") ;
		  if( idxpos < 0 ){
			 gLoginUserEmail=ret;
			 MyCookies.set("email",ret);
		  }
