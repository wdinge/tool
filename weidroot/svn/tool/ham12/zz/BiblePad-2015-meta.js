...
function body_onload() {
	...
	jQuery(window).bind('beforeunload', function(e) {
		...
		return ""+i+" of data saved. It's ready to exit.";
	});  	
...
function AddIntoHistory(sTxt, BookMark) {
	...
	MyCookies.Set("BookMark",getBookMarksInHistory(30).toString().replace(/[,]/g,"|") );
}
...
function construeItem_OnClick() {//on td 0 clicked, change the translation. 
    ...
    case "MyNotes"://
		var src="src='./MyNotes.htm?fname="+sky+"'";
...
function GetCurBible() {
    var BibleVersion = MyCookies.Get("selMainBible");//$("#selMainBible").val(); //GetBibleVersionThruWinLoc();
