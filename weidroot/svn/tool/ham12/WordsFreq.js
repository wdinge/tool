...
UniqObjFreq.prototype.collect_Hebrew_Chars=function(sTxt){//letters
    this.TotCollections++;
    for( var i=0; i<sTxt.length; i+=1) {
...
UniqObjFreq.prototype.collect_Hebrew_Individual_Words=function(sTxt){
    ...
    for( var i=0; i<arr.length; i+=1) {
        ...
        var sword=Remove_Prounce_Hebrew(sword2);
...
UniqObjFreq.prototype.collect_Greek_Letters=function(sTxt){
    this.TotCollections++;
    for( var i=0; i<sTxt.length; i+=1) {
        var sword = sTxt.charAt(i);
        var ccode = sword.charCodeAt(0);
