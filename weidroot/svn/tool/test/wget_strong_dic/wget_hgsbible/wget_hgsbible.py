...
def wget_files(OutputFiles, chtype, iMax):
    ...
    path="hebrew-old-testament"
    indx=0
    hometable="<table border='1' class='hometable'>"
    hometable+="<tr><th>#</th><th>Name</th><th>chap</th></tr>"
    for indx in range(1,66+1): 
        if indx==40:
            path="greek-new-testament"
        bookname=BookChaperMax[indx][0]
        iChapMax=BookChaperMax[indx][1]
        
        bookidx=("00"+str(indx))[-2:]
        print indx, bookname, iChapMax
        selOpn="<select bookidx='"+bookidx+BookChaperMax[indx][2]+"'><option></option>\r\n"
        for i in range(1, 1+iChapMax):
            urfile = bookname+""+str(i)+".html"
            chap=("00"+str(i))[-3:]
            fname =  bookidx +"_"+ chap + BookChaperMax[indx][2] +".html"
            selOpn+="<option value='"+chap+"'>"+str(i)+"</option>\r\n" 
            OutputFiles.append(fname)
            url= "http://www.qbible.com/"+path+urfile
            #print url
            cmd="wget "+url +" -O " + DirTmp + "/" +fname
            print cmd
            if bDownload=="y":
                os.system(cmd)
        selOpn+="</select>\r\n"
        hometable+="<tr class='"+path+"'><td>"+str(indx)+"</td><td>"+bookname[1:-1]+"</td><td>"+selOpn+"</td></tr>"       
        print '------------------------'
        time.sleep(.1) 
...
indxfile="index_hgs.htm"
xtm=wget_files(OutputFiles,"", 0)
...
Dest="../../../../../___bigdata/___zcvf2xf/res/hgsbible"
os.system(cmd)
