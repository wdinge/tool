...
{   
    ...
    var pinyin = me.innerHTML;//.replace(' ', ''); //replace(/^\s+|\s+$/g, ''); 
    ...
    if ( pinyin.length>=2 ) {
        if ( lastChar == last2Char ) { //repeat last char, increment tone.
            me.m_tone+=1;       
            me.innerHTML = me.innerHTML.substring(0, pinyin.length-1);  //remove last repeated.
            pinyin = me.innerHTML;    
            me.m_iStartIndx = 0 ;
    ...
    me.innerHTML = me.innerHTML.replace(/\s+/g, ''); //remove all spaces.
...
function TextareaKeyDwn(evt){
    ...
    if ( 8 == keyCod ) {//backspace. do not delete TextArea until pinyin is gone.
        var spinyin = document.getElementById("pinyininput").value;
        if ( spinyin.length>0 ) {
            document.getElementById("pinyininput").value = spinyin.substring(0,spinyin.length-1);
            return false;         
        }
        return true;
    }
