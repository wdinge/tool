...
function TextareaKeyDwn(textareaObj, sPinyinDispID, sSelectionTableID, sToneID){
    ...
    if( (keyCod>=65 && keyCod<=90) || 32 == keyCod){
      ...
      TextareaKeyDwn_Display(document.m_PinyinInputObj, document.m_SelectionTable, document.m_ToneObj, c );
...
/////////////////////////////////////////
function TextareaKeyDwn_Display(PinyinInputObj, SelectionTableObj, ToneObj, cha)
{   
    ...
    var MAX_SHOW_LEN=10;
    if ( me.value.length>MAX_SHOW_LEN) {
        me.value="";//clear.
        return;
    }
    //NOTE: "b c d f j k l m p q r s t w x y z "; //these are only used at first in pinyin. Carefuly choose 10 of them in orders.
    var CODES_FIRST_ONLY_TAB  = "s d f r b c j k l m ";//p q t w x y z "; //these are only used at first in pinyin. 
