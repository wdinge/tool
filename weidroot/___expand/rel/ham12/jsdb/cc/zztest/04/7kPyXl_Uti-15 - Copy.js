...
{       
    ...
    document.m_ToneObj.innerHTML =GetTot()+"<br>"+MaxLen()+"<br>"+GetAll() ;
...
function TextareaKeyDwn(textareaObj, sPinyinDispID, sSelectionTableID, sToneID){
    ...
    document.m_ToneID = document.getElementById(sToneID);
    ...
    if( (keyCod>=65 && keyCod<=90) || 32 == keyCod){
      ...
      TextareaKeyDwn_Display(document.m_PinyinInputObj, document.m_SelectionTable, c );
      doGetCaretPosition(textareaObj);
      return false;
    }
    if( 8 == keyCod ) {//backspace. do not delete TextArea until pinyin is gone.
      var spinyin = document.m_PinyinInputObj.value;
      if( spinyin.length>0 ){
         document.m_PinyinInputObj.value = spinyin.substring(0,spinyin.length-1);
         return false;         
      }
      return true;
    }
...
/////////////////////////////////////////
function TextareaKeyDwn_Display(PinyinInputObj, SelectionTableObj, cha)
{   
    me = PinyinInputObj; //input element.
        ...
        if ( 2 == pinyin.length ) { //select commonly followed words.
            ...
            if ( iPos >=0  ) {
                ...
                if ( undefined != cfws ) {
                    ...
                    document.m_ToneObj.innerHTML = ""; //cfws + me.m_FlagMore;//+"<br>"+CODES_FOLLOWED;    
    ...
    document.m_ToneObj.innerHTML = me.m_tone;    //+"<br>"+ChineseChars+me.m_FlagMore+"<br> b c d f h j k l m p q r s t w x y z";
...
{       
   ...
   //validate size check
   var tb = tableObj.childNodes[0];;//document.m_SelectionTable.childNodes[0];
   var tds;
